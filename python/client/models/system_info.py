from __future__ import annotations
import datetime
from collections.abc import Callable
from dataclasses import dataclass, field
from kiota_abstractions.serialization import AdditionalDataHolder, Parsable, ParseNode, SerializationWriter
from typing import Any, Optional, TYPE_CHECKING, Union

if TYPE_CHECKING:
    from .system_info_config import SystemInfo_config

@dataclass
class SystemInfo(AdditionalDataHolder, Parsable):
    # Stores additional data not described in the OpenAPI description found when deserializing. Can be used for serialization as well.
    additional_data: dict[str, Any] = field(default_factory=dict)

    # The build_date property
    build_date: Optional[datetime.datetime] = None
    # The config property
    config: Optional[SystemInfo_config] = None
    # The go_version property
    go_version: Optional[str] = None
    # The platform property
    platform: Optional[str] = None
    # The version property
    version: Optional[str] = None
    
    @staticmethod
    def create_from_discriminator_value(parse_node: ParseNode) -> SystemInfo:
        """
        Creates a new instance of the appropriate class based on discriminator value
        param parse_node: The parse node to use to read the discriminator value and create the object
        Returns: SystemInfo
        """
        if parse_node is None:
            raise TypeError("parse_node cannot be null.")
        return SystemInfo()
    
    def get_field_deserializers(self,) -> dict[str, Callable[[ParseNode], None]]:
        """
        The deserialization information for the current model
        Returns: dict[str, Callable[[ParseNode], None]]
        """
        from .system_info_config import SystemInfo_config

        from .system_info_config import SystemInfo_config

        fields: dict[str, Callable[[Any], None]] = {
            "build_date": lambda n : setattr(self, 'build_date', n.get_datetime_value()),
            "config": lambda n : setattr(self, 'config', n.get_object_value(SystemInfo_config)),
            "go_version": lambda n : setattr(self, 'go_version', n.get_str_value()),
            "platform": lambda n : setattr(self, 'platform', n.get_str_value()),
            "version": lambda n : setattr(self, 'version', n.get_str_value()),
        }
        return fields
    
    def serialize(self,writer: SerializationWriter) -> None:
        """
        Serializes information the current object
        param writer: Serialization writer to use to serialize this model
        Returns: None
        """
        if writer is None:
            raise TypeError("writer cannot be null.")
        writer.write_datetime_value("build_date", self.build_date)
        writer.write_object_value("config", self.config)
        writer.write_str_value("go_version", self.go_version)
        writer.write_str_value("platform", self.platform)
        writer.write_str_value("version", self.version)
        writer.write_additional_data_value(self.additional_data)
    

